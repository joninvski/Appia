...
public class StableStorage {
  ...
  public void clean(int rank, long seqno) {
    ...
    while ((h.first != null) && (h.first.seqno <= seqno)) {
      ...
      if (debugOn) {
        debug("Cleaned "+h.first.seqno+" from "+rank);
      }
  ...
  private static final PrintStream debugOut=null;
  
  private void debug(String s) {
    debugOut.println("[StableStorage]: "+s);
  }
  ...
  private void debug() {
    ...
    for (i=0 ; i < storage.length ; i++) {
      ...
      debugOut.println("\t DEBUG STORAGE rank "+i);
      
      for (aux=storage[i].first ; aux != null ; aux=aux.next) {
        debugOut.println("\t\t DEBUG STORAGE seqno "+aux.seqno);
      }
    }
  }
