...
public class GroupBottomSession extends Session {
  ...
  private void handleUpGroupSendableEvent(GroupSendableEvent ev) {
    ...
    try {
      ...
      if (groupHash != groupHashCode) {
        debug("discarded event ("+ev+") due to bad \"group\"");
        return;
      }
      ...
      if (viewHash != viewHashCode) {
        ...
        if (isNext) {
          if (debugFull)
        	  debug("buffering event from possible next view (hash="+viewHash+").");
        ...
        for (int i=0 ; i < vs.previous.length ; i++) {
          if (viewHash == vs.previous[i].hashCode()) {
            debug("discarded event because it belonged to previous view");
            return;
          }
        }
        ...
        debug("received event from same \"group\", but different \"view id\". Sent OtherView. "+ ev.getClass().getName());
      ...
      if ((ev.orig < 0) || (ev.orig >= vs.view.length) && (ev.orig == ls.my_rank)) {
          debug("Event discarded due to bad origin "+ev.orig);
          return;
        }
      ...
      if (ev instanceof Send) {
        ...
        if (!bitset.get(ls.my_rank)) {
          debug("send event discarded because it wasn't for me ("+ev+")");
          return;
        }
      }
      ...
      if (debugFull)
      	debug("Received message "+ev+" from "+ev.orig);
      
      try {
      	ev.go();
      } catch (AppiaEventException ex) {
      	ex.printStackTrace();
      	debug("up event discarded");
      }
    } catch (MessageException ex) {
      debug("Event (\""+ev.getClass().getName()+"\") discarded because i couldn't read headers.");
      return;
    }
  }
  ...
  private void handleDownGroupSendableEvent(GroupSendableEvent ev) {
	if ((vs == null) || (vs.view.length == 1)) {
      debug("Trying to send to somenone but i'm the only one. Discarding Event");
      return;
    }
    ...
    if (isSend) {
      ...
      for (int i=0 ; i < dests.length ; i++) {
        if((dests[i] >= vs.addresses.length) || (dests[i] < 0))
          debug("Invalid destination rank ("+dests[i] +") in Send. Event ("+ev.getClass().getName()+ ") Discarded.");
        bitset.set(dests[i]);
      }
    }
    ...
    if (debugFull)
      debug("Sending message (isSend="+isSend+") "+ev);
  ...
  private void handleView(View ev) {
    ...
    if (debugFull)
    	debug("Recived new view: group="+groupHashCode+" view="+viewHashCode);
  ...
  private void handleGroupEvent(GroupEvent ev) {
    if (!vs.group.equals(ev.group) || !vs.id.equals(ev.view_id)) {
      debug("event (\""+ev.getClass().getName()+"\") going ("+ev.getDir()+") discarded due to bad group or view_id");
      return;
    }
  ...
  private void sendOtherViews(int state, Object other_addr, Object multicast_addr, Channel channel) {
    try {
      ...
      ex.printStackTrace();
      debug("Impossible to send OtherViews ("+state+")");
    }
  }
  ...
  private void debug(String s) {
    if ((debug != null) && (debugFull || debugOn))
      debug.println("appia:group:GroupBottomSession["+(ls != null ? ls.my_rank : -1)+"/"+(vs != null ? vs.view.length : -1)+"]: "+s);
  }
