...
import org.continuent.appia.protocols.group.events.Uniform;
import org.continuent.appia.protocols.group.intra.NewView;
...
public class VSyncSession extends Session {
  ...
  private void handleBlock(Block ev) {
    ...
    if (syncCoord == ls.my_rank) {
      ...
      if (type != BLOCKED) {
        debug("Invalid Block type (received="+type+", expected="+BLOCKED+") from "+ev.orig+" to coordinator");
        return;
      }
      ...
      if (type != BLOCK) {
        debug("Invalid Block type (received="+type+", expected="+BLOCK+") from "+ev.orig+" to other");
        return;
      }
  ...
  private void handleSync(Sync ev) {
    ...
    if (syncCoord == ls.my_rank) {
      ...
      if (type != SYNCHED) {
        debug("Invalid Sync type (received="+type+", expected="+SYNCHED+") from "+ev.orig+" to coordinator");
        return;
      }
      ...
      if (type != SYNC) {
        debug("Invalid Sync type (received="+type+", expected="+SYNC+") from "+ev.orig+" to other");
        return;
      }
      ...
      if (debugFull) {
        debug("{"+ls.my_rank+"} Received Sync from "+ev.orig);
  ...
  private void handleGroupSendableEvent(GroupSendableEvent ev) {
    ...
    if (ev instanceof Send) {
      if (ev.getDir() == Direction.DOWN) {
        ...
        if ((global_sends != null) && (my_sends_recv[ev.orig] == global_sends[ev.orig])) {
          debug("Send Event from "+ev.orig+" (failed="+ls.failed[ev.orig]+") discarded because it was greater than global");
          return;
        }
    ...
    if (ev.getDir() == Direction.DOWN) {
      ...
      if ((global_casts != null) && (my_casts[ev.orig] == global_casts[ev.orig])) {
        debug("Cast Event from "+ev.orig+" (failed="+ls.failed[ev.orig]+") discarded because it was greater than global");
      }
      else{
        my_casts[ev.orig]++;
        try { ev.go(); } catch (AppiaEventException ex) { ex.printStackTrace(); }
  ...
  private void coord_handleSync(int orig, long[] casts, Channel channel) {
    if (sync == null) {
      debug("Received Sync out of time. Ignored.");
      return;
    }
  ...
  private void in_block(Channel channel) {
    ...
    if (debugFull) {
      debug("{"+ls.my_rank+"} Sending my_casts and my_sends_sent to coordinator");
      debugLongArray("my_casts=",my_casts);
  ...
  private void in_sync(Channel channel) {
    if (debugFull) {
      debug("testing in_sync");
      debug("in_sync: my_casts==global_casts -> "+equals(my_casts,global_casts,null));
      debug("in_sync: my_sends_recv== global_sends -> "+equals(my_sends_recv,global_sends,ls.failed));
    }
  ...
  private void end_blocking(Channel channel) {
    ...
    if (debugFull) {
      debug("{"+ls.my_rank+"} Ended blocking started synching");
      debugLongArray("global_casts=",global_casts);
  ...
  private void debug(String s) {
    if ((debugFull || debugOn) && (debug != null))
      debug.println("appia:group:VSyncSession: "+s);
  }
  
  private void debugLongArray(String s, long[] a) {
    if ((debugFull || debugOn) && (debug != null)) {
      debug.print("appia:group:VSyncSession: "+s+"[");
